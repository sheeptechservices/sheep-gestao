...
// ── Task Form Modal ───────────────────────────────────────────────────────────
interface FormState { title: string; description: string; urgency: TaskUrgency | ''; done: boolean; assigned_to: string; week_id: string | null; project_id: string; flags: string[]; flag_comment: string; deadline: string }
...
function TaskFormModal({ task, projectId, initialWeekId, onSave, onClose, onDelete, weeks, projects }: TaskFormModalProps) {
  ...
  const [form, setForm] = useState<FormState>({
    ...
    week_id:      task?.week_id      ?? initialWeekId ?? null,
    ...
    >
      <div
        ...
        style={{
          ...
          width: 560, boxShadow: '0 24px 60px rgba(0,0,0,0.18)',
          animation: 'modalSlideUp 0.22s ease both',
      ...
      >
        ...
        <div>
          ...
          <textarea
            ...
            placeholder="Detalhes adicionais sobre o entregável..."
            rows={3}
        ...
        {/* Project — shown when editing any task OR when creating a general task */}
        {(!!task || !projectId) && (
          <div>
            <label style={{ fontSize: 10, fontWeight: 800, color: 'var(--gray2)', textTransform: 'uppercase', letterSpacing: '0.06em', display: 'block', marginBottom: 6 }}>
              Projeto {!projectId && !task && <span style={{ fontWeight: 500, textTransform: 'none', color: 'var(--gray2)' }}>(opcional)</span>}
            </label>
            <AppSelect
              value={form.project_id}
              onChange={v => setForm(f => ({ ...f, project_id: v }))}
              options={[{ value: '', label: '— Nenhum (geral) —' }, ...projects.map(p => ({ value: p.id, label: p.name }))]}
            />
          </div>
        )}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          ...
          <div>
            ...
            <WeekPickerSelect
              value={form.week_id}
              onChange={v => setForm(f => ({ ...f, week_id: v }))}
              weeks={projectWeeks}
            />
          </div>
        </div>
        ...
        <div>
          <label style={{ fontSize: 10, fontWeight: 800, color: 'var(--gray2)', textTransform: 'uppercase', letterSpacing: '0.06em', display: 'block', marginBottom: 6 }}>
            Previsão de entrega <span style={{ fontWeight: 500, textTransform: 'none', color: 'var(--gray2)' }}>(opcional)</span>
          </label>
          <AppDatePicker
            value={form.deadline}
    ...
    <div>
      ...
      {isShowingModal && modalProjectId !== null && (
        <TaskFormModal
          ...
          initialWeekId={editing ? null : addingForWeekId}
          onSave={data => handleSave(modalProjectId, data)}
