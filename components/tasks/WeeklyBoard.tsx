...
export function WeeklyBoardCard({ task, project, isDragging, onDragStart, onDragEnd, onClick, onToggleDone, onDelete, onMembersChange }: {
  ...
  return (
    <div
      ...
      style={{
        background: isDragging ? accentColor + '12' : bloqueado ? 'rgba(220,38,38,0.07)' : atencao ? 'rgba(124,58,237,0.07)' : revisar ? 'rgba(217,119,6,0.07)' : hov ? 'var(--card-hover-bg)' : 'var(--white)',
        borderRadius: 10,
...
export function WeeklyBoard() {
  ...
  // "Sem data definida" — tasks with absolutely no deadline and not done,
  // shown persistently across all weeks since they have no scheduled date
  const noDateTasks: Task[] = applyFilters(
    tasks.filter(t => !t.deadline && !t.done)
  ).sort(byUrgency)
