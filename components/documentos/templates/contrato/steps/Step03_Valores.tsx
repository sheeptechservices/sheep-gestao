...
import type { ContratoData, ItemContrato } from '@/components/documentos/types/contrato'
...
export function Step03_Valores() {
  ...
  const itens: ItemContrato[] = d.itensValor || []

  const total = itens.reduce((acc, i) => acc + (Number(i.valor) || 0), 0)
  const fmt = (n: number) => n.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })

  const addItem = () => set({ itensValor: [...itens, { id: crypto.randomUUID(), descricao: '', valor: 0 }] })
  const removeItem = (id: string) => set({ itensValor: itens.filter(i => i.id !== id) })
  const updateItem = (id: string, field: keyof ItemContrato, val: string | number) =>
    set({ itensValor: itens.map(i => i.id === id ? { ...i, [field]: val } : i) })
      ...
      <FormSection title="Composição do valor">
        ...
        {itens.length > 0 && (
          <div style={{
            ...
            <span style={{ fontSize: 12, fontWeight: 800, color: 'var(--black)' }}>Total</span>
